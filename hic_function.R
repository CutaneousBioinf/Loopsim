...
psorLoci = function(loopfile) {
  ...
  loopfile_forSearch = data.table(chr=uni_inter_out$V1, start=uni_inter_out$V2, end=uni_inter_out$V3)
  check = foverlaps(loopfile_forSearch, ploci_ref, type="any", which = T)
  overlap_indice = check[!is.na(check$yid),1]$xid
  psor_indice = unique(check$yid[!is.na(check$yid)])
  
  return(list(n_interval=nrow(uni_inter_out), n_overlap=length(overlap_indice), psor_indice=psor_indice))
}
